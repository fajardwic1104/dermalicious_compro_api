...
const corsOptions = {
  origin: [
    "https://dermalicious.id",
    "https://www.dermalicious.id",
    "http://103.177.56.115",
  ], // Allowed origins
  methods: ["GET", "POST", "PUT", "DELETE"],
  allowedHeaders: ["Content-Type", "Authorization"],
...
app.post("/api/login", async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ id_user: user.id_user }, "your_jwt_secret", {
      expiresIn: "1h",
    });
    res.json({ token });
  } catch (err) {
...
app.post("/api/register", async (req, res) => {
  ...
  try {
    ...
    const [insertResult] = await db.query(
      "INSERT INTO users (name_user, email_user, password, created_at, updated_at) VALUES (?, ?, ?, NOW(), NOW())",
      [name_user, email_user, hashedPassword]
    );

    const userId = insertResult.insertId;
    await db.query(
      "INSERT INTO tbl_role_user (id_role, id_user) VALUES (?, ?)",
      [id_role, userId]
    );
...
app.put("/api/users/:id_user", async (req, res) => {
  ...
  try {
    ...
    if (password) {
      ...
      await db.query(
        "UPDATE users SET name_user = ?, email_user = ?, password = ?, updated_at = NOW() WHERE id_user = ?",
        [...updates, id_user]
      );
    } else {
      await db.query(
        "UPDATE users SET name_user = ?, email_user = ?, updated_at = NOW() WHERE id_user = ?",
        [...updates, id_user]
      );
    }
...
app.delete("/api/users/:id", async (req, res) => {
  ...
  try {
    ...
    await connection.query("DELETE FROM tbl_role_user WHERE id_user = ?", [
      userId,
    ]);
...
app.get("/api/program-catering", async (req, res) => {
  try {
    const [results] = await db.query(
      "SELECT id, title, calories, image, description FROM program_catering WHERE status_delete = 0"
    );
    res.json(results);
  } catch (err) {
...
app.post("/api/program-catering", upload.single("image"), async (req, res) => {
  ...
  const imageUrl = `http://api.dermalicious.id/uploads/${path.basename(
    imagePath
  )}`;

  try {
    const [result] = await db.query(
      "INSERT INTO program_catering (title, calories, image, description, status_delete) VALUES (?, ?, ?, ?, 0)",
...
app.put(
  ...
  async (req, res) => {
    ...
    try {
      const [result] = await db.query(
        `UPDATE program_catering SET ${updateString}, updated_at = NOW() WHERE id = ?`,
        params
      );
...
app.delete("/api/program-catering/:id", async (req, res) => {
  ...
  try {
    const [result] = await db.query(
      "UPDATE program_catering SET status_delete = 1 WHERE id = ?",
      [id]
    );
    if (result.affectedRows === 0) {
      return res.status(404).json({ error: "Program not found" });
...
app.get("/api/testimonials", async (req, res) => {
  try {
    const [results] = await db.query(
      "SELECT * FROM testimonials WHERE status_delete = 0"
    );
    res.json(results);
  } catch (err) {
    return res.status(500).json({ error: "Database query failed" });
...
app.post("/api/testimonials", upload.single("image"), async (req, res) => {
  ...
  const imageUrl = imagePath
    ? `https://api.dermalicious.id/uploads/${path.basename(imagePath)}`
    : null;

  try {
    const [result] = await db.query(
      "INSERT INTO testimonials (name, role, image, rating, text, status_delete) VALUES (?, ?, ?, ?, ?, 0)",
      [name, role, imageUrl, rating, text]
    );
    res
      .status(201)
      .json({ id: result.insertId, name, role, image: imageUrl, rating, text });
  } catch (err) {
    res.status(500).json({ error: "Database error", details: err });
  }
});
...
app.put("/api/testimonials/:id", upload.single("image"), async (req, res) => {
  ...
  if (req.file) {
    const imageUrl = `https://api.dermalicious.id/uploads/${path.basename(
      req.file.path
    )}`;
    updates.push("image = ?");
    params.push(imageUrl);
  ...
  try {
    const [result] = await db.query(
      `UPDATE testimonials SET ${updateString}, updated_at = NOW() WHERE id = ?`,
      params
    );
    if (result.affectedRows === 0) {
...
app.delete("/api/testimonials/:id", async (req, res) => {
  ...
  try {
    const result = await db.query(
      "UPDATE testimonials SET status_delete = 1 WHERE id = ?",
      [id]
    );
...
app.get("/api/partnerships", async (req, res) => {
  try {
    const [results] = await db.query(
      "SELECT * FROM partnerships WHERE status_delete = 0"
    );
    res.json(results);
  } catch (err) {
    res.status(500).json({ error: "Database query failed" });
...
app.post("/api/partnerships", upload.single("image"), async (req, res) => {
  ...
  try {
    const [result] = await db.query(
      "INSERT INTO partnerships (title, image, created_at, updated_at, status_delete) VALUES (?, ?, NOW(), NOW(), 0)",
      [title, imageUrl]
    );
...
app.put("/api/partnerships/:id", upload.single("image"), async (req, res) => {
  ...
  try {
    const [result] = await db.query(
      `UPDATE partnerships SET ${updateString}, updated_at = NOW() WHERE id = ? AND status_delete = 0`,
      [...params, id]
    );
    if (result.affectedRows === 0) {
      return res
        .status(404)
        .json({ error: "Partnership not found or has been deleted" });
    }
...
app.delete("/api/partnerships/:id", async (req, res) => {
  ...
  try {
    const [result] = await db.query(
      "UPDATE partnerships SET status_delete = 1 WHERE id = ?",
      [id]
    );
    if (result.affectedRows === 0) {
...
app.post("/api/links", async (req, res) => {
  ...
  const query =
    "INSERT INTO links (facebook, instagram, youtube, tiktok, whatsapp, email) VALUES (?, ?, ?, ?, ?, ?)";
...
app.put("/api/links/:id", async (req, res) => {
  ...
  const { facebook, instagram, youtube, tiktok, whatsapp, email } = req.body;
  const query =
    "UPDATE links SET facebook = ?, instagram = ?, youtube = ?, tiktok = ?, whatsapp = ?, email = ? WHERE id = ?";
...
app.get("/api/eazy_meals", async (req, res) => {
  try {
    const [results] = await db.query(
      "SELECT * FROM eazy_meals WHERE status_delete = 0"
    );
    res.json(results);
  } catch (err) {
    res.status(500).json({ error: "Database query failed" });
...
app.get("/api/eazy_meals/:id", async (req, res) => {
  ...
  try {
    const [results] = await db.query(
      "SELECT * FROM eazy_meals WHERE id = ? AND status_delete = 0",
      [id]
    );
    if (results.length === 0)
      return res
        .status(404)
        .json({ error: "Meal not found or has been deleted" });
    res.json(results[0]);
  } catch (err) {
    res.status(500).json({ error: "Database query failed" });
  }
...
app.post("/api/eazy_meals", upload.single("image"), async (req, res) => {
  ...
  try {
    const [result] = await db.query(
      "INSERT INTO eazy_meals (name, image, url, created_at, updated_at, status_delete) VALUES (?, ?, ?, NOW(), NOW(), 0)",
      [name, imageUrl, url]
    );
...
app.put("/api/eazy_meals/:id", upload.single("image"), async (req, res) => {
  ...
  try {
    const [result] = await db.query(
      `UPDATE eazy_meals SET ${updateString}, updated_at = NOW() WHERE id = ? AND status_delete = 0`,
      [...params, id]
    );
    if (result.affectedRows === 0)
      return res
...
app.delete("/api/eazy_meals/:id", async (req, res) => {
  ...
  try {
    const [result] = await db.query(
      "UPDATE eazy_meals SET status_delete = 1 WHERE id = ?",
      [id]
    );
    if (result.affectedRows === 0)
...
app.get("/api/page_banner/:id", async (req, res) => {
  ...
  try {
    const [results] = await db.query("SELECT * FROM page_banner WHERE id = ?", [
      id,
    ]);
    if (results.length === 0)
      return res.status(404).json({ error: "Page Banner not found" });
    res.json(results[0]);
  } catch (err) {
    res.status(500).json({ error: "Database query failed" });
  }
...
app.post("/api/page_banner", upload.single("image"), async (req, res) => {
  ...
  const imagePath = req.file ? req.file.path : null;
  const imageUrl = imagePath
    ? `https://api.dermalicious.id/uploads/pagebanner/${path.basename(
        imagePath
      )}`
    : null;
  ...
  try {
    const [result] = await db.query(
      "INSERT INTO page_banner (name, image, created_at, updated_at) VALUES (?, ?, NOW(), NOW())",
      [name, imageUrl]
    );
  ...
  try {
    ...
    if (req.file) {
      ...
      const [oldResults] = await db.query(
        "SELECT image FROM page_banner WHERE id = ?",
        [id]
      );
      ...
      await db.query(
        "UPDATE page_banner SET name = ?, image = ?, updated_at = NOW() WHERE id = ?",
        [name, imageUrl, id]
      );
      ...
      await db.query(
        "UPDATE page_banner SET name = ?, updated_at = NOW() WHERE id = ?",
        [name, id]
      );
...
app.use(
  express.static(
    path.join(
      __dirname,
      "../home/web/programmer/upload/dermalicious_web/frontend/build"
    )
  )
);
